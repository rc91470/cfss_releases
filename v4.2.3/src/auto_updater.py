...
def check_for_updates_on_startup(root, current_version, repo_owner, repo_name):
    """Function to check for updates on startup - compatible with existing CFSS code"""
    def check_updates():
        ...
        if update_info:
            # Show update dialog
            dialog = UpdateDialog(root, updater, update_info)
            dialog.show()
